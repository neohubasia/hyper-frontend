  ...
  $.ajax({
    ...
    success: async function (result) {
      if (result.status == "SUCCESS" && result.data.length > 0) {
        ...
        if (slider == true) {
          let catSliderClone = $("#commonCategorySlider")
            .find(".owl-carousel")
            .clone();
          let sliderHtml = "";
          ...
          $.each(result.data, async function (Idx, obj) {
            catSliderClone
              ...
              .attr(
                "data-setbg",
                "https://via.placeholder.com/270x270/555/555"
              );
            catSliderClone
              .find("a")
              .attr("href", `/shop-items?category_id=${obj.id}`);
  ...
  $.ajax({
    ...
    success: function (result) {
      if (result && result.status == "SUCCESS") {
        let optionHtml =
          '<option value="#" data-display="-- Select One--"  disabled selected>-- Select One--</option>';

        $.each(result.data, async function (Idx, obj) {
          optionHtml += `<option value=${obj.id} data-display=${
            obj[args.showName]
          }> ${obj[args.showName]} </option>`;
        });
